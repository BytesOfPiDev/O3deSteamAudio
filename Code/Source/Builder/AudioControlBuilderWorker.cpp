...
static constexpr auto JsonEventsKey{ "includedEvents" };
static constexpr auto NodeDoesNotExistMessage =
    "%s node does not exist. Please be sure that you have defined at "
    "least one %s for this Audio Control file.\n";
static constexpr auto MalformedNodeMissingAttributeMessage{
...
namespace SteamAudio
{
    ...
    AZ_TYPE_INFO_WITH_NAME_IMPL(
        ...
        "SteamAudioControlBuilderWorker",
        "{635BD743-EE48-458F-B549-BD56C02861B9}");
    ...
    namespace Internal
    {
        ...
        namespace Legacy
        {
                ...
                AZStd::vector<AZStd::string>& banksReferenced) -> AZ::Outcome<void, AZStd::string>
            {
                ...
                while (preloadRequestNode)
                {
                    ...
                    {
                        auto bopFileNode = configGroupNode->first_node(XmlTags::SteamAudioFileTag);
                        if (!bopFileNode)
                        {
                            return AZ::Failure(AZStd::string::format(
                                MalformedNodeMissingChildNodeMessage,
            ...
            AZStd::vector<AZStd::string>& banksReferenced) -> AZ::Outcome<void, AZStd::string>
        {
            ...
            while (preloadRequestNode)
            {
                ...
                if (auto bopAudioFileNode =
                        preloadRequestNode->first_node(XmlTags::SteamAudioFileTag))
                {
                    while (bopAudioFileNode)
                    {
                        ...
                        bopAudioFileNode =
                            bopAudioFileNode->next_sibling(XmlTags::SteamAudioFileTag);
                    }
                }
                else
                {
        ...
        auto GetEventsFromBankMetadata(
            rapidjson::Value const& rootObject,
            AZStd::set<AZStd::string>& eventNames) -> AZ::Outcome<void, AZStd::string>
        {
        ...
        auto GetEventsFromBank(
            AZStd::string const& bankMetadataPath,
            AZStd::set<AZStd::string>& eventNames) -> AZ::Outcome<void, AZStd::string>
        {
        ...
        AssetBuilderSDK::CreateJobsResponse& response)
    {
        ...
        for (AssetBuilderSDK::PlatformInfo const& info : request.m_enabledPlatforms)
        {
            ...
            descriptor.m_jobKey = "Steam Audio Control";
