...
namespace SteamAudio
{
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////
    WavFileParser::~WavFileParser() = default;

    ///////////////////////////////////////////////////////////////////////////////////////////////
    auto WavFileParser::ParseHeader(AZ::IO::FileIOStream& fileStream) -> size_t
    {
        if (IsHeaderValid())
        {
        ...
        auto* writePtr = reinterpret_cast<AZ::u8*>(&m_header);
        ...
        while (!dataTagFound)
        {
            ...
            if (ValidTag(header.tag, WavFileParser::fmt__tag))
            {
                ...
                writePtr += sizeof(m_header.fmt.header); // skip forward because it was already read
                                                         // into the temp chunkheader.
            ...
            if (fileStream.GetCurPos() == fileStream.GetLength())
            {
                AZ_Error(
                    "WavFileParser",
                    false,
                    "WavFileParser::ParseHeader - Got to end of file and did not locate a 'data' "
                    "chunk!\n");
                return 0;
            }
        }
        ...
        {
            AZ_Error(
                "WavFileParser", false, "WavFileParser::ParseHeader - Did not find a 'fmt' tag!\n");
        }
        ...
        if (!ValidTag(m_header.data.tag, WavFileParser::data_tag))
        {
            AZ_Error(
                "WavFileParser",
                false,
                "WavFileParser::ParseHeader - Did not find a 'data' tag!\n");
        }
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////
    auto WavFileParser::GetSampleType() const -> Audio::AudioInputSampleType
    {
        switch (m_header.fmt.audioFormat)
        {
        case 1:
            return Audio::AudioInputSampleType::Int;
        case 3:
            return Audio::AudioInputSampleType::Float;
        default:
            return Audio::AudioInputSampleType::Unsupported;
        }
    }

    ///////////////////////////////////////////////////////////////////////////////////////////////
    // static
    auto WavFileParser::ValidTag(const AZ::u8 tag[4], const AZ::u8 name[4]) -> bool
    {
